...
const metricsRetentionInput = document.getElementById("metrics-retention-days");
...
saveBtn.addEventListener("click", async () => {
  ...
  const metrics_retention_days = Number(metricsRetentionInput.value);
  const res = await api("/api/v1/admin/settings", {
    method: "PUT",
    body: JSON.stringify({ retention_days, session_days, metrics_retention_days }),
  });
  if (!res.ok) {
  ...
  metricsRetentionInput.value = data.metrics_retention_days;
...
runRetentionBtn.addEventListener("click", async () => {
  ...
  show(
    retentionMsg,
    `Deleted ${data.deleted} log(s)` +
      (data.metrics_deleted != null ? `, ${data.metrics_deleted} metric row(s).` : ".")
  );
});
...
async function boot() {
  ...
  if (meData.setup_required) {
    window.location.href = "/setup";
    return;
  }
  navUser.textContent = meData.username || "admin";
  if (meData.auth_mode === "anonymous" && navBadge) {
